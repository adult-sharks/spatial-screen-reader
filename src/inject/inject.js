...
// Get last element in element tree when hover
// Get text content from last element
  ...
  if (elems[elems.length - 1] != undefined) {
    ...
    if (elems[elems.length - 1].textContent != "") {
      ...
      if (elems[elems.length - 1].attributes.getNamedItem("alt")) {
        var text = "image: " + elems[elems.length - 1].attributes.getNamedItem("alt").value;
      } else {
        var text = "image";
      }
    }
...
// When mouse is moved, logKey function is called and in function, Set mouse coordinate and run TTS 
...
// core logic //
////////////////
// When it starts, Add CSS of cursor and detect mousemove 
const startCycle = () => {
  console.log("mouse on");
  changeCursor();
  mouseEventListener();
};
// When it stops, reset Cursor and remove mousemove, close TTS
const stopCycle = () => {
